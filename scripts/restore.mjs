...
import { TABLES, fetchTable, requireEnv, supabaseHeaders } from "./backup-tables.mjs";
...
// out of both the comparison and the write.
function comparable(row, match) {
  const copy = { ...row };
...
async function upsert(table, match, rows) {
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    const batch = rows.slice(i, i + BATCH_SIZE).map((row) => comparable(row, match));
...
for (const { name, match } of TABLES) {
  ...
  for (const row of backup) {
    ...
    else if (!sameRow(comparable(row, match), comparable(current, match))) changed.push(row);
  }
  const extra = live.filter((row) => !backupKeys.has(matchKey(row, match))).length;

  plan.push({ name, match, writes: [...missing, ...changed] });
  console.log(
...
if (total === 0) {
  ...
  for (const { name, match, writes } of plan) {
    if (writes.length > 0) await upsert(name, match, writes);
  }
  console.log(`\nRestored ${total} rows.`);
}
